...
% Create a subplot that fills the whole figure
ax = subplot('Position', [0 0 1 1]);

% Show the image in the subplot
imshow(img, 'Parent', ax); % Show image
...
normalizedCellArray = cellfun(normalize, equalLengthintensities, 'UniformOutput', false);
...
Circles = cell(size(r_special2));
for i = 1:length(Circles)
    M = normalizedCellArray{i, 1}; %radius pertubations
    X = length(M); % number of points in the array
    current_radius = r_special2(i);

    % Generate generic circle
    myArray = linspace(0, 1, X); % for simplicity, this array goes from 0 to 1 with X points
    % generate the angles for each point in myArray
    angles = 2 * pi * myArray - pi/2; % convert the array to range between 0 and 2pi (360 degrees)
    
    % generate the 2D coordinates for each point
    Y = zeros(X, 2); % initialize an empty 2D array with X rows and 2 columns
    Y(:, 1) = (current_radius+M*size(img,1)/100) .* cos(angles)' + optic_disc_center(1); % X-coordinates
    Y(:, 2) = (current_radius+M*size(img,1)/100) .* sin(angles)' + optic_disc_center(2); % Y-coordinates
    
    hold on
    plot(Y(:,1),Y(:,2),'w', 'LineStyle', ':')
    Circles{i} = Y;
end
